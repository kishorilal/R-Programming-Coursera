attrition_DownSampling<-function(){
  #setwd( "C:/Users/kishor/Documents/specdata/employee")
  library(readxl)
  ...
  '%ni%'<-Negate("%in%")
  library("caret")
    down_Sample<-downSample(x=trn[,colnames(trn) %ni% "Attrition"],y=trn$Attrition)
  
  glm_model<-glm(Class ~.,data=down_Sample,family = "binomial");
  res<- predict(glm_model,down_Sample,type = "response")
  ...
  library("xlsx")
  for(i in cut_off) {
    d1<-table(ActualValue=down_Sample$Class,PredictValue=res>i)
    #print(d1)
    if(NCOL(d1)>1){
  ...
  print(result)
  write.xlsx(result,file = "attrition.xlsx", sheetName = "downsampling", 
